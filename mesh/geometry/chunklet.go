	...
	for x := float32(0); x < chunk.Size().X; x++ {
		for y := index; y < index+ChunkletSize; y++ {
			...
			for z := float32(0); z < chunk.Size().Z; z++ {
				...
				if y > 0 {
					bottomBlock = chunk.GetBlockAt(iX, iY-1, iZ)
				}
				...
				if z == 0 {
					southBlock = south.GetBlockAt(iX, iY, int(south.Size().Z)-1)
				} else {
					southBlock = chunk.GetBlockAt(iX, iY, iZ-1)
				}
				...
				if z == chunk.Size().Z-1 {
					northBlock = north.GetBlockAt(iX, iY, 0)
				} else {
					northBlock = chunk.GetBlockAt(iX, iY, iZ+1)
				}
				...
				if x == 0 {
					westBlock = west.GetBlockAt(int(west.Size().X)-1, iY, iZ)
				} else {
					westBlock = chunk.GetBlockAt(iX-1, iY, iZ)
				}
				...
				if x == chunk.Size().X-1 {
					eastBlock = east.GetBlockAt(0, iY, iZ)
				} else {
					eastBlock = chunk.GetBlockAt(iX+1, iY, iZ)
				}
